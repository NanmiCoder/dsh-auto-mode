...
- **Ask for confirmation:** interrupt the user only when the target, effect, or intent remains unclear, or when Shell semantics use a dynamic target, a nested interpreter, a stateful terminal, or a construct that cannot be read statically.
- **Deny immediately:** destructive access to filesystem roots, user home, DSH_HOME, system or credential-critical paths, privilege escalation, permission-system bypasses, and obvious credential exfiltration. These code-level circuit breakers cannot be overridden by the classifier model, and they apply to every segment of a compound command line, so `&&`, `;`, pipelines, and redirection cannot smuggle a protected target past them. Windows paths additionally normalize extended/NT namespaces, drive-relative forms, trailing dots/spaces, and 8.3 system-directory aliases, while device namespaces and reserved device names such as `CON`, `NUL`, and `COM1` are rejected.
...
Every Bash and PowerShell call is inspected. A command line is first split on `&&`, `||`, `;`, pipelines, newlines, and redirection, then judged segment by segment: the deterministic fast path requires every segment to be a statically recognized safe operation, and any other combination enters semantic classification instead of prompting merely because the line contains `&&` or `2>&1`. Only constructs that genuinely cannot be read statically — command substitution, here-documents, shell grouping, unbalanced quotes, dynamic deletion or redirection targets, nested shells, and inline-code execution — require human approval and cannot be cleared by the classifier.
