...
- **请求确认**：只有目标、效果或用户意图仍然不清楚，或者 Shell 含动态、嵌套、状态化或无法可靠解析的语义时才打断用户。
- **直接拒绝**：破坏磁盘根目录、用户目录、DSH_HOME、系统或凭据关键路径，工作区外递归删除，绕过权限系统，以及明显的凭据外传。这些代码级熔断不能被分类模型覆盖。Windows 路径会额外规范化扩展/NT 命名空间、盘符相对路径、尾随点空格和 8.3 系统目录别名，并拒绝设备命名空间与 `CON` / `NUL` / `COM1` 等保留设备名。
- **识别临时产物**：记录当前 Session 中成功创建的精确路径，允许安全清理本次会话创建的项目内或临时目录产物。
- **分类器故障时关闭失败路径**：交互模式回退到人工确认；无人值守模式没有审批通道时拒绝执行。

所有 Bash 和 PowerShell 调用都会经过检查。只有一个由静态 token 构成的简单命令可以进入确定性快速路径；管道、变量、重定向、glob、命令替换、嵌套 Shell、编码命令和无法证明安全的语法都必须人工确认，分类器不能越过这一限制。
