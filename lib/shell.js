...
function readOnlyCommand(name, tokens, shell) {
    if (shell === 'bash') {
        if (['pwd', 'ls', 'rg', 'grep', 'head', 'tail', 'cat', 'wc', 'od', 'du', 'stat', 'file', 'which', 'type'].includes(name))
            return true;
        if (name === 'sed')
...
export function assessShell(source, shell, roots, artifacts, owner) {
    ...
    if (parsed === undefined)
        return manualReview(`${shell} command is not one static simple command`);
    const name = commandName(parsed.tokens[0]);
    if (nestedInterpreter(name))
        return manualReview('nested shell execution requires manual review');
    const deletion = deletionSpec(name, parsed.tokens, shell);
    if (deletion !== undefined) {
