...
export function parseSimpleCommand(source, shell) {
    ...
    if (input === '' || /[\n\r;&|<>(){}\[\]*?]/.test(input))
        return undefined;
    if (shell === 'bash' && (input.includes('$') || input.includes('`')))
        return undefined;
    if (shell === 'pwsh' && (input.includes('$') || input.includes('`') || /%[^%]+%/.test(input)))
        return undefined;
    const tokens = [];
    let token = '';
    let quote;
...
export function hardDenyShellReason(source, shell, roots) {
    ...
    for (const target of deletion.targets) {
        ...
        if (protectedReason !== undefined)
            return `destructive operation targets ${protectedReason}`;
...
function deletionSpec(name, tokens, shell) {
...
function readOnlyCommand(name, tokens, shell) {
    if (shell === 'bash') {
        if (['pwd', 'ls', 'rg', 'grep', 'head', 'tail', 'wc', 'stat', 'file', 'which', 'type'].includes(name))
            return true;
        if (name === 'sed')
            return tokens.includes('-n');
        if (name === 'git')
            return ['status', 'diff', 'log', 'show', 'rev-parse'].includes(tokens[1]?.toLowerCase() ?? '');
        return false;
    }
    return ['get-location', 'get-childitem', 'get-content', 'select-string', 'get-item', 'test-path'].includes(name);
}
function creationSpec(name, tokens, shell, roots) {
    let raw;
    if (shell === 'bash' && ['mkdir', 'touch'].includes(name))
        raw = tokens.slice(1).filter(token => !token.startsWith('-'));
...
export function assessShell(source, shell, roots, artifacts, owner) {
    ...
    if (parsed === undefined)
        return authorization(`${shell} command is not one static simple command`);
    const name = commandName(parsed.tokens[0]);
    if (nestedInterpreter(name))
        return authorization('nested shell execution requires manual review');
    const deletion = deletionSpec(name, parsed.tokens, shell);
    if (deletion !== undefined) {
        if (deletion.targets.length === 0)
            return ambiguous('deletion target could not be determined');
