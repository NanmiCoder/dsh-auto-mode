...
function nestedInterpreter(name, words) {
    if (['sh', 'bash', 'zsh', 'fish', 'ksh', 'dash', 'cmd', 'cmd.exe', 'powershell', 'powershell.exe', 'pwsh', 'pwsh.exe',
        'eval', 'exec', 'source', '.', 'iex', 'invoke-expression', 'invoke-command', 'start-process'].includes(name))
        return true;
...
function workspacePathsAreRoutine(words, roots) {
...
function readOnlyCommand(name, words, shell) {
    ...
    if (shell === 'bash') {
        ...
        if (name === 'find')
            return !tokens.some(token => /^-(?:delete|exec|execdir|ok|okdir|fprint|fprintf|fls)$/.test(token));
        if (name === 'git')
...
function assessSegment(segment, shell, roots, artifacts, owner) {
    ...
    if (nestedInterpreter(name, words))
        return manualReview('nested shell or inline-code execution requires manual review');
