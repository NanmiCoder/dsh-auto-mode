...
function pwshAssignment(words) {
    const first = words[0];
    const second = words[1];
    if (first === undefined || second === undefined)
        return undefined;
    if (!first.dynamic || !PWSH_VARIABLE.test(first.text))
        return undefined;
    if (second.text !== '=' || second.dynamic || second.glob || second.quoted)
        return undefined;
...
function assessSegment(segment, shell, roots, artifacts, owner) {
    ...
    if (shell === 'pwsh') {
        ...
        if (assignment !== undefined) {
            ...
            return assessSegment({ words: rhs, writeTargets: segment.writeTargets, readTargets: segment.readTargets }, shell, roots, artifacts, owner);
        }
    }
    if (first.dynamic)
